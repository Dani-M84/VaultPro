...
def derive_key(pw, salt):
    return PBKDF2(pw.encode(), salt, dkLen=32, count=200000)
...
def enc(data, key):
    ...
    raw = pad(raw.decode('latin1')).encode('latin1')
...
def dec(encd, key):
    raw = base64.b64decode(encd)
    iv, ct = raw[:16], raw[16:]
    cipher = AES.new(key, AES.MODE_CBC, iv)
    pt = cipher.decrypt(ct)
    pt = unpad(pt.decode('latin1'))
...
class LoginScreen(Screen):
    ...
    def do_login(self,*a):
        try:
            ...
            self.msg.text=f"Error: {str(e)[:50]}"
